...
def get_lung_mask_convex_hull(in_lung_seg_mask):
    convex_mask = np.copy(in_lung_seg_mask)

    for i_layer in range(in_lung_seg_mask.shape[2]):
        mask_layer = in_lung_seg_mask[:, :, i_layer]
        if np.sum(mask_layer) > 0:
            convex_mask_layer = convex_hull_image(mask_layer)
            convex_mask[:, :, i_layer] = convex_mask_layer
        else:
            convex_mask[:, :, i_layer] = 0

    return convex_mask


def get_cardiac_roi_mask(in_lung_seg_path, out_roi_mask_path, kaggle_format):
    in_lung_seg_obj = nib.load(in_lung_seg_path)
    in_lung_seg_mask = in_lung_seg_obj.get_data()
    lung_mask = None
    if kaggle_format:
        lung_mask = np.where(
            (in_lung_seg_mask == 2) | (in_lung_seg_mask == 4),
            1, 0
        )
    else:
        lung_mask = in_lung_seg_mask

    convex_mask = get_lung_mask_convex_hull(lung_mask)

    cardiac_roi_mask = convex_mask - lung_mask

    out_roi_mask_obj = nib.Nifti1Image(cardiac_roi_mask, header=in_lung_seg_obj.header, affine=in_lung_seg_obj.affine)
    nib.save(out_roi_mask_obj, out_roi_mask_path)
