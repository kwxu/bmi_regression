...
def get_registration_command(registration_method_name, reg_args, reg_tool_root, fixed_image, moving_image, output_image, output_mat):
    ...
    if registration_method_name == 'affine_flirt':
        ...
        command_list.append(f'{linearBCVslow_path} -F {fixed_image} -M {moving_image} -O {output_mat}')
        command_list.append(f'{deedsBCVslow_path} {reg_args} -F {fixed_image} -M {moving_image} -O {output_image} -A {actual_output_mat_path}')
        ...
        command_list.append(f'{deedsBCV_path} {reg_args} -F {fixed_image} -M {moving_image} -O {output_image} -A {actual_output_mat_path}')
    else:
...
def replace_nan(in_img_path, out_img_path, val):
    im_obj = nib.load(in_img_path)
    im_data = im_obj.get_data()

    logger.info(f'Replace nan to value {val}')

    new_im_data = np.nan_to_num(im_data, nan=val)
    new_im_obj = nib.Nifti1Image(new_im_data, header=im_obj.header, affine=im_obj.affine)

    logger.info(f'Save non-nan image to {out_img_path}')
    nib.save(new_im_obj, out_img_path)
