...
class GradCAM(_BaseWrapper):
    ...
    def _print_tensor_min_max(self, data):
        print(data.shape)
        ...
        print(gcam.min(dim=1, keepdim=True)[0].cpu().numpy())
        print(gcam.max(dim=1, keepdim=True)[0].cpu().numpy())


    def generate(self, target_layer):
        fmaps = self._find(self.fmap_pool, target_layer)
        # self._print_tensor_min_max(fmaps)
        grads = self._find(self.grad_pool, target_layer)
        # self._print_tensor_min_max(grads)
        weights = F.adaptive_avg_pool3d(grads, 1)
        # weights = self._compute_grad_weights(grads)
        # self._print_tensor_min_max(weights)

        gcam = torch.mul(fmaps, weights).sum(dim=1, keepdim=True)
        # self._print_tensor_min_max(gcam)
        gcam = F.relu(gcam)
        # self._print_tensor_min_max(gcam)

        print(gcam.shape)
        #
        # B, C, H, W, D = gcam.shape
        # gcam = gcam.view(B, -1)
        # gcam -= gcam.min(dim=1, keepdim=True)[0]
        # gcam /= gcam.max(dim=1, keepdim=True)[0]
        #
        ...
        # gcam = gcam.view(B, C, H, W, D)

        print('heatmap dimension.')
        print(gcam.shape)

        gcam = F.interpolate(
            gcam, self.image_shape, mode="trilinear", align_corners=False
        )
